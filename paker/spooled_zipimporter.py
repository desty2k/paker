...
import tempfile  # for storing zip contents
...
class zipimporter:
    ...
    def __init__(self, content):
        self.fp = tempfile.SpooledTemporaryFile(mode="w+b", suffix=".zip", max_size=-1)
        self.fp.write(content)
        self.fp.seek(0)
        self.content = content
        self.logger = logging.getLogger(self.__class__.__name__)

        files = _read_directory(self.fp)
        self._files = files
    ...
    def find_loader(self, fullname, path=None):
        ...
        if _is_dir(self, modpath):
            ...
            return None, [f'{path_sep}{modpath}']
        return None, []
    ...
    def find_module(self, fullname, path=None):
        ...
        """
        if self.fp.closed:
            raise ZipImportError("zip object closed")
        self.logger.debug("Searching for {} in {}".format(fullname, path))
    ...
    def get_data(self, pathname):
        ...
        try:
            toc_entry = self._files[pathname]
        except KeyError:
            raise OSError(0, '', pathname)
        return _get_data(self.fp, toc_entry)
    ...
    def get_source(self, fullname):
        ...
        try:
            ...
            return None
        return _get_data(self.fp, toc_entry).decode()
    ...
    def load_module(self, fullname):
        ...
        self.logger.info("Loading {}".format(fullname))
        code, ispackage, modpath = _get_module_code(self, fullname)
        ...
        try:
            if ispackage:
                mod.__path__ = [_get_module_path(self, fullname)]
        ...
        self.logger.info("{} loaded successfully".format(mod.__name__))
        return mod
...
def _get_module_info(self, fullname):
    ...
    for suffix, isbytecode, ispackage in _zip_searchorder:
        ...
        if fullpath in self._files:
            self.logger.debug("Found {} in files".format(fullpath))
            return ispackage
    return None
...
def _get_data(fp, toc_entry):
    ...
    try:
        ...
        raise ZipImportError("can't read Zip file")
    buffer = fp.read(30)
    if len(buffer) != 30:
        raise EOFError('EOF read where not expected')
    ...
    if buffer[:4] != b'PK\x03\x04':
        ...
        raise ZipImportError('bad local file header')
...
def _get_module_code(self, fullname):
    ...
    for suffix, isbytecode, ispackage in _zip_searchorder:
        fullpath = path + suffix
        _bootstrap._verbose_message('trying {}{}{}', self.fp, path_sep, fullpath, verbosity=2)
        try:
            toc_entry = self._files[fullpath]
        except KeyError:
            pass
            ...
            modpath = toc_entry[0]
            data = _get_data(self.fp, toc_entry)
            if isbytecode:
